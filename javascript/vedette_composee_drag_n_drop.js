...
// +-------------------------------------------------+
// $Id: vedette_composee_drag_n_drop.js,v 1.5 2015-12-10 11:14:44 ngantier Exp $ */
...
function vedette_composee_available_fields_vedette_composee_subdivision(dragged,target){
	...
	caller_property_name=subdivision_id.substring(0,n)+"_composed";
	caller_property_authid=dragged.getAttribute("authid");
	caller_property_authlabel=dragged.getAttribute("dragtext");
	...
	if ((dragged.getAttribute("parentorder") == target.getAttribute("parentorder")) && ((target.getAttribute("cardmax") == "") || (target.getAttribute("cardmax") > nb_elements))) {
		...
		vedette_element.create_box(caller_property_name,dragged.getAttribute("vedettetype"), div, target.getAttribute("subdivisiontype"), new_order, 0, "", "", target.getAttribute("parentorder"));
...
function vedette_composee_element_vedette_composee_subdivision(dragged,target){
	...
	// On vérifie qu'on change de subdivision, sinon on ne fait rien
		if (dragged_parent != target) {
			vedette_element.update_box(dragged.getAttribute("vedettetype"), dragged, target.getAttribute("subdivisiontype"), new_order, target.getAttribute("parentorder"));
