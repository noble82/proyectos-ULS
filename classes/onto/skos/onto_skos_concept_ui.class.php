...
// +-------------------------------------------------+
// $Id: onto_skos_concept_ui.class.php,v 1.22 2015-12-04 14:58:41 vtouchard Exp $
	...
	public static function get_list_selector($controler,$params){
		...
		if($elements["nb_total_elements"]){
			foreach($elements['elements'] as $uri => $item){
				...
				$current_element_form = str_replace("!!item!!", htmlentities(addslashes((isset($item[substr($lang,0,2)]) ? $item[substr($lang,0,2)] : $item['default'])),ENT_QUOTES,$charset), $current_element_form);
