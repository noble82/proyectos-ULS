...
// +-------------------------------------------------+
// $Id: nomenclature_musicstand_ui.js,v 1.38 2016-01-05 09:10:06 vtouchard Exp $
		    	...
		    	switch(evt_type){
		    		...
		    		case "new_workshop" :
		    			if(this.musicstand.get_hash().indexOf(evt_args.nomenclature_hash)!=-1 && this.musicstand.get_used_by_workshops()){
		    				if(!this.musicstand.indefinite_effective){
		    					var input_effective = dom.byId(this.dom_node.id+'_'+this.musicstand.get_id()+'_input_effective');
		    					if(input_effective.value == this.musicstand.family.nomenclature.indefinite_character){
		    						input_effective.value = 1;
		    					}
		    					input_effective.value = parseInt(input_effective.value)+1;
			    				this.musicstand.set_effective(parseInt(input_effective.value));
		    				}else if(this.musicstand.get_effective() == 0){ //premier ws ajouté & undef
		    					var input_effective = dom.byId(this.dom_node.id+'_'+this.musicstand.get_id()+'_input_effective');
		    					this.musicstand.displayed_effective = this.musicstand.get_effective()+1;
		    					this.musicstand.set_effective(parseInt(this.musicstand.effective)+1);
		    					input_effective.value = this.musicstand.family.nomenclature.indefinite_character;
		    				}else{
		    					var input_effective = dom.byId(this.dom_node.id+'_'+this.musicstand.get_id()+'_input_effective');
		    					this.musicstand.displayed_effective = this.musicstand.get_effective()+1;
		    					this.musicstand.set_effective(parseInt(this.musicstand.effective)+1);
		    					input_effective.value = this.musicstand.displayed_effective;
		    					this.musicstand.set_indefinite_effective(false);
		    				}  /**
		    					* TODO: Ajouter cas ou on en ajoute un apres l'indefinite effective.
		    					*/
//		    				var input_effective = dom.byId(this.dom_node.id+'_'+this.musicstand.get_id()+'_input_effective');
//		    				if(this.musicstand.displayed_effective == "~"){
//		    					this.musicstand.displayed_effective = this.musicstand.get_effective();
//		    					this.musicstand.set_indefinite_effective(false);
//		    				}
//		    				this.musicstand.set_effective(parseInt(this.musicstand.effective)+1);
//		    				input_effective.value = this.musicstand.displayed_effective;
		    		...
		    		case "workshop_state_changed":
		    			if(this.musicstand.get_hash().indexOf(evt_args.nomenclature_hash)!=-1 && this.musicstand.get_used_by_workshops()){
		    				var input_effective = dom.byId(this.dom_node.id+'_'+this.musicstand.get_id()+'_input_effective');
		    				if(evt_args.workshop_undefined){
		    					this.musicstand.set_indefinite_effective(true);
		    					this.musicstand.set_effective(1);
		    					this.displayed_effective = this.musicstand.family.nomenclature.indefinite_character;
		    					input_effective.value = this.displayed_effective;
		    					...
		    					var input_effective = dom.byId(this.dom_node.id+'_'+this.musicstand.get_id()+'_input_effective');
		    					this.musicstand.displayed_effective = this.musicstand.get_effective();
		    					this.musicstand.set_effective(parseInt(this.musicstand.effective));
		    					input_effective.value = this.musicstand.displayed_effective;
		    					this.musicstand.set_indefinite_effective(false);
		    				}
		    				topic.publish("musicstand_ui","musicstand_changed", {
...
//	    			this.get_input_effective().value = this.musicstand.get_effective();
		    		topic.publish('workshops_ui', 'workshops_changed', {
			    		...
			    		is_undefined: this.musicstand.is_indefinite_effective()
			    	});
		    	}
