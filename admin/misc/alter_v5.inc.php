...
// +-------------------------------------------------+
// $Id: alter_v5.inc.php,v 1.664 2016-01-13 15:41:42 apetithomme Exp $
...
switch ($action) {
	...
	case "v5.14":
		...
		case "v5.20":
			...
			// VT & DG - Ajout de la table memorisant les grilles de saisie de formulaire des autorités
			// grille_auth_type : Type d'autorité
			// grille_auth_filter : Signature (en cas de grilles multiples pour une autorité)
			// grille_auth_descr_format : Format JSON de la grille
			$rqt = "create table if not exists grilles_auth (
				grille_auth_type VARCHAR(32) not null default '',
				grille_auth_filter VARCHAR(255) null default '',
				grille_auth_descr_format mediumblob NOT NULL,
				PRIMARY KEY (grille_auth_type,grille_auth_filter)
				)";
			echo traite_rqt($rqt,"create table if not exists grilles_auth");
...
/* A mettre en 5.21
			...
			// Antoine : ajout des colonnes pour enrichissement des auteurs
			$rqt="alter table authors add author_enrichment text , add author_enrichment_last_update datetime not null default '0000-00-00'";
			echo traite_rqt($rqt,"alter table authors add author_enrichment,author_enrichment_last_update");

			if (pmb_mysql_num_rows(pmb_mysql_query("select 1 from parametres where type_param= 'opac' and sstype_param='enrichment_bnf_sparql' "))==0){
				$rqt = "INSERT INTO parametres (id_param, type_param, sstype_param, valeur_param, comment_param, section_param, gestion)
				VALUES (0, 'opac', 'enrichment_bnf_sparql', '0', 'Enrichissement des autorites avec data.bnf.fr','a_general',0)";
				echo traite_rqt($rqt,"insert opac_enrichment_bnf_sparql into parametres");
			}

			// DG - Juste pour ceux qui se sont récemment mis à jour en dev
			// Suppression de la table des statuts de concept
			// Script de suppression des champs num_statut
			// <bordel DG>
			$rqt ="drop table concept_statuts";
			echo traite_rqt($rqt,"drop table concept_statuts");
			$rqt = "alter table authors drop num_statut";
			echo traite_rqt($rqt,"alter table authors drop num_statut");
			$rqt = "alter table noeuds drop num_statut";
			echo traite_rqt($rqt,"alter table noeuds drop num_statut");
			$rqt = "alter table publishers drop num_statut";
			echo traite_rqt($rqt,"alter table publishers drop num_statut");
			$rqt = "alter table collections drop num_statut";
			echo traite_rqt($rqt,"alter table collections drop num_statut");
			$rqt = "alter table sub_collections drop num_statut";
			echo traite_rqt($rqt,"alter table sub_collections drop num_statut");
			$rqt = "alter table series drop num_statut";
			echo traite_rqt($rqt,"alter table series drop num_statut");
			$rqt = "alter table titres_uniformes drop num_statut";
			echo traite_rqt($rqt,"alter table titres_uniformes drop num_statut");
			$rqt = "alter table indexint drop num_statut";
			echo traite_rqt($rqt,"alter table indexint drop num_statut");
			$rqt = "alter table authperso_authorities drop num_statut";
			echo traite_rqt($rqt,"alter table authperso_authorities drop num_statut");
			// </bordel DG>
			...
			echo traite_rqt($rqt,"alter table responsability_tu add primary key");
			...
			if (pmb_mysql_num_rows(pmb_mysql_query("select 1 from parametres where type_param= 'pmb' and sstype_param='authors_qualification' "))==0){
				$rqt="INSERT INTO parametres (id_param, type_param, sstype_param, valeur_param, comment_param, section_param, gestion)
					VALUES (NULL, 'pmb', 'authors_qualification', '0', 'Activer qualification d\'un lien d\'auteur dans les notices et les titres uniformes\n 0 : Non\n 1 : Oui', '', '0')";
				echo traite_rqt($rqt,"insert pmb_authors_qualification into parametres ");
			}
			...
			if (pmb_mysql_num_rows(pmb_mysql_query("select 1 from parametres where type_param= 'pmb' and sstype_param='scan_request_activate' "))==0){
				...
				echo traite_rqt($rqt,"insert scan_request_activate into parametres");
			}
			...
			$rqt = "create table if not exists scan_request_status(
				...
				scan_request_status_list_editable int(1) not null default 0,
			...
			if (pmb_mysql_num_rows(pmb_mysql_query("select 1 from parametres where type_param= 'opac' and sstype_param='scan_request_status' "))==0){
				$rqt = "INSERT INTO parametres (id_param, type_param, sstype_param, valeur_param, comment_param, section_param, gestion)
					VALUES (0, 'opac', 'scan_request_status', '1', 'Statut par défaut en création de demande de numérisation à l\'OPAC','a_general',1)";
				echo traite_rqt($rqt,"insert opac_scan_request_status into parametres");
			}
