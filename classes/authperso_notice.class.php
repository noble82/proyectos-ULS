...
// +-------------------------------------------------+
// $Id: authperso_notice.class.php,v 1.15 2016-01-04 10:56:38 apetithomme Exp $
		...
		foreach($this->onglets_auth_list as $onglet_num => $onglet){
			...
			foreach($onglet as $authperso_num => $auth_perso){
				foreach($auth_perso as $auth_num => $auth){
					...
					if(!$new)	$aff.=", ";
					$aff.= '<a href="'.$pmb_url_base.'autorites.php?categ=see&sub=authperso&id='.$auth['id'].'">'.$auth['isbd'].'</a>';	
					$new=0;
				}
			}
		}	
	...
	function get_form(){
		...
		$authperso_notice_elt_tpl_num="									
				  	<div id='drag_authperso_!!authperso_id!!_!!iauthperso!!'  class='row' dragtype='authperso!!authperso_id!!' draggable='yes' recept='yes' recepttype='authperso!!authperso_id!!' handler='handle_!!authperso_id!!_!!iauthperso!!'		
				        ...
				        <input type='text' class='saisie-80emr' id='f_authperso_!!authperso_id!!_!!iauthperso!!' name='f_authperso_!!authperso_id!!_!!iauthperso!!' data-form-name='f_authperso_!!authperso_id!!_' completion='authperso_!!authperso_id!!' value=\"!!authperso_libelle!!\" completion=\"authpersoories_mul\" autfield=\"f_authperso_id_!!authperso_id!!_!!iauthperso!!\" />
		...
		if (count($onglet_used)) {
			$req="SELECT * FROM notice_onglet where id_onglet not in(".implode(",",$onglet_used).")";
