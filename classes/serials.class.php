...
// +-------------------------------------------------+
// $Id: serials.class.php,v 1.182 2015-12-21 11:11:36 ngantier Exp $
...
*/
	...
	function serial_delete() {
		...
		serial::delete_vedette_links($this->serial_id);
	...
	static function delete_vedette_links($id) {	
		...
		if (pmb_mysql_num_rows($res_responsability)) {
			while($r=pmb_mysql_fetch_object($res_responsability)){
				...
				switch($type_aut){
					case 0:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'serial_authors'), $object_id, TYPE_NOTICE_RESPONSABILITY_PRINCIPAL);
						break;
					case 1:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'serial_authors'), $object_id,TYPE_NOTICE_RESPONSABILITY_AUTRE);
						break;
					case 2:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'serial_authors'), $object_id,TYPE_NOTICE_RESPONSABILITY_SECONDAIRE);
						break;
				}
			}
		}
...
class bulletinage extends serial {
	...
	var $b_notice_show_expl=1; // affichage des exemplaires dans la notice de bulletin
	...
	function make_display() {
		...
		if ($this->bull_num_notice) {
			if($this->b_notice_show_expl) {
				...
				$m_display=new mono_display($this->bull_num_notice,5,'',0,'','','',0,0,0);
			}
	...
	function delete_analysis () {	
		...
		if($this->bulletin_id) {
			...
			while(($dep = pmb_mysql_fetch_object($myQuery2))) {
				...
				// Clean des vedettes
				analysis::delete_vedette_links($dep->analysis_notice);
	...
	static function delete_vedette_links($id) {	
		...
		if (pmb_mysql_num_rows($res_responsability)) {
			while($r=pmb_mysql_fetch_object($res_responsability)){
				...
				switch($type_aut){
					case 0:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'bulletin_authors'), $object_id, TYPE_NOTICE_RESPONSABILITY_PRINCIPAL);
						break;
					case 1:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'bulletin_authors'), $object_id,TYPE_NOTICE_RESPONSABILITY_AUTRE);
						break;
					case 2:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'bulletin_authors'), $object_id,TYPE_NOTICE_RESPONSABILITY_SECONDAIRE);
						break;
				}
			}
		}
