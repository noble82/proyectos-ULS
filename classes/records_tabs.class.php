...
// +-------------------------------------------------+
// $Id: records_tabs.class.php,v 1.4 2016-01-05 17:26:53 apetithomme Exp $
...
class records_tabs {
	...
	protected function get_tab_records_indexed_with_concept(){
		...
		if(count($vedette_composee_found)){
			...
			$query = 'select count(distinct notice_id) from notices join index_concept on index_concept.num_object = notices.notice_id and index_concept.type_object = "'.TYPE_NOTICE.'" where index_concept.num_concept in ('.implode(',', $concept_ids).') ';
			...
			if ($nb_results && ($quoi == $tab->get_name())) {
				$query = 'select distinct notice_id from notices join index_concept on index_concept.num_object = notices.notice_id and index_concept.type_object = "'.TYPE_NOTICE.'" where index_concept.num_concept in ('.implode(',', $concept_ids).') ';
