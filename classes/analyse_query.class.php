...
// +-------------------------------------------------+
// $Id: analyse_query.class.php,v 1.63 2015-12-07 14:26:22 vtouchard Exp $
// MB Version 2: MAJ
		...
		if($objects_ids){
			...
			if($this->input !== "*"){
				...
				if(count($words)){
					...
					//INSEE remplacement de straight_join par join 
					$pert_query_words = "select ".$field_id." as ".$final_id.", max(pond * (!!pert!!)) as pert from words join ".$table_mot." on num_word = id_word where ";
					$where = "";
