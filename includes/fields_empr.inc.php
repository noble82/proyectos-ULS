...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.115 2015-12-23 14:10:49 dgoron Exp $
...
$aff_filter_list_empr=array("text"=>"aff_filter_text_empr","list"=>"aff_filter_list_empr","query_list"=>"aff_filter_query_list_empr","query_auth"=>"aff_filter_query_auth_empr","date_box"=>"aff_filter_date_box_empr","comment"=>"aff_filter_comment_empr","external"=>"aff_filter_external_empr","url"=>"aff_filter_resolve_empr","resolve"=>"aff_filter_resolve_empr","marclist"=>"aff_filter_marclist_empr","html"=>"aff_filter_comment_empr","text_i18n"=>"aff_filter_text_i18n_empr","q_txt_i18n"=>"aff_filter_q_txt_i18n_empr");
...
function aff_query_auth_empr($field,&$check_scripts,$script="") {
	...
	for ($i=0; $i<$n; $i++) {
		...
		$ret.="<input type='text' att_id_filter='".$id_thes_unique."' completion='$completion' autfield='".$field["NAME"]."_$i' class='saisie-50emr' id='f_".$field["NAME"]."_$i'  persofield='".$field["NAME"]."' autfield='".$field["NAME"]."_$i' name='f_".$field["NAME"]."_$i'  data-form-name='f_".$field["NAME"]."_'  $readonly value=\"".htmlentities($isbd,ENT_QUOTES,$charset)."\" />\n";
		$ret.="<input type='hidden' id='".$field["NAME"]."_$i' name='".$field["NAME"]."_$i' data-form-name='".$field["NAME"]."_' value=\"".htmlentities($values[$i],ENT_QUOTES,$charset)."\">";
...
function aff_query_auth_empr_search($field,&$check_scripts,$varname) {
	...
	$val_dyn=3;
	$ret.="<input type='text'  completion='$completion' autfield='".$field["NAME"]."' class='saisie-50emr' id='f_".$field["NAME"]."'  persofield='".$field["NAME"]."' autfield='".$field["NAME"]."' name='f_".$field["NAME"]."' data-form-name='f_".$field["NAME"]."' $readonly $fnamevar_id value=\"".htmlentities($libelle,ENT_QUOTES,$charset)."\" />\n";
...
function aff_date_box_empr($field,&$check_scripts) {
	...
	foreach ($values as $value) {
		...
		$ret .= "<div>
					<input type='hidden' id='".$field[NAME]."_val_".$count."' name='".$field[NAME]."[]' data-form-name='".$field[NAME]."_val_' value='$val' />
					<input class='bouton' type='button' id='".$field[NAME]."_lib_".$count."'  name='".$field[NAME]."_lib_".$count."'  data-form-name='".$field[NAME]."_lib_' value='".($val_popup?formatdate($val_popup):htmlentities($msg["parperso_nodate"],ENT_QUOTES,$charset))."' onClick=\"openPopUp('".$base_path."/select.php?what=calendrier&caller='+this.form.name+'&date_caller=".$val_popup."&param1=".$field[NAME]."_val_".$count."&param2=".$field[NAME]."_lib_".$count."&auto_submit=NO&date_anterieure=YES', 'date_".$field[NAME]."', 250, 300, -2, -2, 'toolbar=no, dependent=yes, resizable=yes')\" />&nbsp;
	...
	if ($options[REPEATABLE][0][value]) {
		...
		$ret .= "<script>
			function add_custom_date_box_(field_id, field_name, value, value_popup) {
				...
				lib.setAttribute('id', field_name + '_lib_' + count);
				if (value_popup) {
...
function aff_text_empr($field,&$check_scripts) {
	...
	foreach ($values as $avalues) {
		$ret.="<input id=\"".$field[NAME].$count."\" type=\"text\" size=\"".$options[SIZE][0][value]."\" maxlength=\"".$options[MAXSIZE][0][value]."\" name=\"".$field[NAME]."[]\" data-form-name='".$field[NAME]."' value=\"".htmlentities($avalues,ENT_QUOTES,$charset)."\">";
		if ($options[REPEATABLE][0][value] && !$count)
...
function aff_comment_empr($field,&$check_scripts) {
	...
	foreach ($values as $avalues) {
		$ret.="<textarea id=\"".$field[NAME]."_".$count."\"' cols=\"".$options[COLS][0][value]."\"  rows=\"".$options[ROWS][0][value]."\" maxlength=\"".$options[MAXSIZE][0][value]."\" name=\"".$field[NAME]."[]\" data-form-name='".$field[NAME]."_' wrap=virtual>".htmlentities($avalues,ENT_QUOTES,$charset)."</textarea>";
		if ($options[REPEATABLE][0][value] && !$count) {
...
function aff_list_empr($field,&$check_scripts,$script="") {
	...
	if ($options["AUTORITE"][0]["value"]!="yes") {
		...
		$readonly='';
		for ($i=0; $i<$n; $i++) {
			$ret.="<input type='text' class='saisie-50emr' id='f_".$field["NAME"]."_$i' completion='perso_".$_custom_prefixe_."' persofield='".$field["NAME"]."' autfield='".$field["NAME"]."_$i' name='f_".$field["NAME"]."_$i' data-form-name='f_".$field["NAME"]."_' $readonly value=\"".htmlentities($libelles[$i],ENT_QUOTES,$charset)."\" />\n";
			$ret.="<input type='hidden' id='".$field["NAME"]."_$i' name='".$field["NAME"]."_$i' data-form-name='".$field["NAME"]."_' value=\"".htmlentities($values[$i],ENT_QUOTES,$charset)."\">";
...
function aff_url_empr($field,&$check_scripts){
	...
	foreach ($values as $avalues) {
		...
		$ret.= $msg['persofield_url_link']."<input id='".$field[NAME]."_link$count' type='text' class='saisie-30em' name='".$field[NAME]."[link][]' data-form-name='".$field[NAME]."_link' onchange='cp_chklnk_".$field["NAME"]."($count,this);' value='".htmlentities($avalues[0],ENT_QUOTES,$charset)."'>";
		$ret.=" <input class=\"bouton\" type='button' value='".$msg['persofield_url_check']."' onclick='cp_chklnk_".$field["NAME"]."($count,this);'>";
		//$ret.="<br />";
		$ret.="&nbsp;".$msg['persofield_url_linklabel']."<input id='".$field[NAME]."_linkname$count' type='text' class='saisie-15em' size='".$options[SIZE][0][value]."' name='".$field[NAME]."[linkname][]' data-form-name='".$field[NAME]."_linkname' value='".htmlentities($avalues[1],ENT_QUOTES,$charset)."'>";
		if ($options[REPEATABLE][0][value] && !$count)
...
function aff_html_empr($field,&$check_scripts) {
	...
	foreach ($values as $avalues) {
		if($pmb_editorial_dojo_editor){
			$ret.="<input type='hidden' name='".$field[NAME]."[$count]' id='".$field[NAME]."_$count' data-form-name='".$field[NAME]."_' value=''/>";
			...
			$ret.="<textarea id='".$field[NAME]."_".$count."' name='".$field[NAME]."[]' data-form-name='".$field[NAME]."_' width='".$options[WIDTH][0][value]."px' height='".$options[HEIGHT][0][value]."px' wrap='wrap' style='display:inline-block;'>".$avalues."</textarea>";
