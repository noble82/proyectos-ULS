...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.203 2016-01-12 13:53:57 jpermanne Exp $
...
class mterm {
	...
	public function mterm($ufield,$op,$values,$vars,$inter,$uid="") {
		$this->uid = $uid;
		$this->ufield=$ufield;
		$this->op=$op;
		$this->values=$values;
...
class search {
    ...
    public function search($rec_history=false,$fichier_xml="",$full_path='') {
				...
				for ($j=0; $j<count($field); $j++) {
					...
					for ($z=0; $z<count($q)-1; $z++) {
						...
						//Si la saisie est un ISBN
    						...
    						if(($q[$z]['CLASS'] == "searcher_records_all_fields")){//Pour savoir si la recherche tous champs inclut les docnum ou pas
    						...
    						if(is_array($q[$z]['FIELDSRESTRICT'])) {
...
//   							print "<br><br>".$main;
 ...
 // fonction de calcul de la visibilite d'un champ de recherche
    ...
    public function show_form($url,$result_url,$result_target='',$memo_url='') {
    	global $charset;
    	global $search;
    	global $add_field;
    	global $delete_field;
    	...
    	if(!$this->full_path){
	    	if ($this->fichier_xml!="") {
	    		if (file_exists($include_path."/search_queries/".$this->fichier_xml."_subst.xml")) {
	    			$fp=fopen($include_path."/search_queries/".$this->fichier_xml."_subst.xml","r") or die("Can't find XML file");
	    			$size=filesize($include_path."/search_queries/".$this->fichier_xml."_subst.xml");
	    		} else {
	    			$fp=fopen($include_path."/search_queries/".$this->fichier_xml.".xml","r") or die("Can't find XML file");
	    			$size=filesize($include_path."/search_queries/".$this->fichier_xml.".xml");
	    		}
	    	} else {
    		...
    		if (file_exists($this->full_path.$this->fichier_xml."_subst.xml")) {
    			$fp=fopen($this->full_path.$this->fichier_xml."_subst.xml","r") or die("Can't find XML file");
    			$size=filesize($this->full_path.$this->fichier_xml."_subst.xml");
    		} else {
    			$fp=fopen($this->full_path.$this->fichier_xml.".xml","r") or die("Can't find XML file");
    			$size=filesize($this->full_path.$this->fichier_xml.".xml");
    		} 
    	}		
	...
	protected function get_display_icons($nb_results, $recherche_externe = false) {
		...
		if ($this->rec_history) {
			...
			if ($current!==false) {
				$tri_id_info = $_SESSION["tri"] ? "&sort_id=".$_SESSION["tri"] : "";
				$display_icons .= "&nbsp;<a href='#' onClick=\"openPopUp('./print_cart.php?current_print=$current&action=print_prepare$tri_id_info','print',600,700,-2,-2,'scrollbars=yes,menubar=0,resizable=yes'); return false;\"><img src='./images/basket_small_20x20.gif' border='0' align='center' alt=\"".$msg["histo_add_to_cart"]."\" title=\"".$msg["histo_add_to_cart"]."\"></a>&nbsp;<a href='#' onClick=\"openPopUp('./print.php?current_print=$current&action_print=print_prepare$tri_id_info','print',500,600,-2,-2,'scrollbars=yes,menubar=0'); w.focus(); return false;\"><img src='./images/print.gif' border='0' align='center' alt=\"".$msg["histo_print"]."\" title=\"".$msg["histo_print"]."\"/></a>";
				$display_icons .= "&nbsp;<a href='#' onClick=\"openPopUp('./download.php?current_download=$current&action_download=download_prepare".$tri_id_info."','download',500,600,-2,-2,'scrollbars=yes,menubar=0'); return false;\"><img src='./images/upload.gif' border='0' align='center' alt=\"".$msg["docnum_download"]."\" title=\"".$msg["docnum_download"]."\"/></a>";
				if ($pmb_allow_external_search){
					...
					$display_icons .= "&nbsp;<a ".$tag_a."  title='".$msg["connecteurs_external_search_sources"]."'><img src='./images/external_search.png' border='0' align='center' alt=\"".$msg["connecteurs_external_search_sources"]."\"/></a>";
				}
				if ($nb_results<=$pmb_nb_max_tri) {
