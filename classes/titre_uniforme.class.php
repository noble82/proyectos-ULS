...
// +-------------------------------------------------+
// $Id: titre_uniforme.class.php,v 1.90 2015-12-31 10:39:18 ngantier Exp $
...
class titre_uniforme {
	...
	function titre_uniforme($id=0,$recursif=0) {
		if($id) {
			// on cherche à atteindre un titre uniforme existant
			$this->recursif=$recursif;
			$this->id = $id;
			$this->getData();
		} else {
			// le titre uniforme n'existe pas
			$this->id = 0;
			$this->getData();
		}
	}
	...
	function gen_oeuvre_expression_form() {
		...
		for($i = 0; $i < $max_oeuvre_expression; $i ++) {
			...
			if (sizeof ( $this->oeuvre_expressions ) == 0) {
				$ptab_expression = str_replace ( '!!expression_type!!', $this->get_selector('expression', 'f_oeuvre_expression_type0', ''), $ptab_expression );
				...
				$ptab_expression = str_replace ( '!!expression_type!!', $this->get_selector('expression', 'f_oeuvre_expression_type'.$i, $this->oeuvre_expressions[$i]['type']), $ptab_expression );
				$ptab_expression = str_replace ( '!!oeuvre_expression_code!!', $this->oeuvre_expressions [$i] ["to_id"], $ptab_expression );
				$ptab_expression = str_replace ( '!!oeuvre_expression!!', htmlentities ( $this->oeuvre_expressions [$i] ["to_name"], ENT_QUOTES, $charset ), $ptab_expression );
			}
			$oeuvre_expression_repetables .= $ptab_expression;
		}
		$tpl = "
		$oeuvre_expression_tpl
		<input type='hidden' name='max_oeuvre_expression' value=\"!!max_oeuvre_expression!!\" />
	...
	function gen_oeuvre_event_form() {
		...
		$tpl = "
		$oeuvre_event_tpl
		<input type='hidden' name='max_oeuvre_event' value=\"!!max_oeuvre_event!!\" />
	...
	function update_oeuvre_expression($value) {
		global $dbh;
	...
	function delete_oeuvre_expression() {
		global $dbh;
		if(!$this->id) return;
		$rqt_del = "delete from tu_oeuvres_links where oeuvre_link_from='".$this->id."' and oeuvre_link_other_link = 0 and oeuvre_link_expression = 1";
	...
	function gen_other_link_form() {
		...
		for($i = 0; $i < $max_other_link; $i ++) {
			...
			if (sizeof ( $this->other_links ) == 0) {
				...
				$ptab_expression = str_replace ( '!!other_link_code!!', $this->other_links [$i] ["to_id"], $ptab_expression );
				$ptab_expression = str_replace ( '!!other_link!!', htmlentities ( $this->other_links [$i] ["to_name"], ENT_QUOTES, $charset ), $ptab_expression );
				$ptab_expression = str_replace ( '!!link_type!!', $this->get_selector('other_link', 'f_oeuvre_other_link'.$i, $this->other_links[$i]["type"]), $ptab_expression );
			}
			$other_link_repetables .= $ptab_expression;
		}
		$tpl = "
		$other_link_tpl
		<input type='hidden' name='max_other_link' value=\"!!max_other_link!!\" />
	...
	function update_other_link($value) {
		global $dbh;
	...
	static function delete_vedette_links($id) {	
		...
		$rqt_responsability = 'select id_responsability_tu from responsability_tu where responsability_tu_num="'.$id.'" ';
		...
		if (pmb_mysql_num_rows($res_responsability)) {
			while($r=pmb_mysql_fetch_object($res_responsability)){
				...
				switch($type_aut){
					case 0:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'titre_uniforme'), $object_id, TYPE_TU_RESPONSABILITY);
						break;
					case 1:
						vedette_link::delete_vedette_link_from_object(new vedette_composee(0,'titre_uniforme'), $object_id, TYPE_TU_RESPONSABILITY_INTERPRETER);
						break;
				}
			}
		}
	...
	public function get_selector($type, $name, $selected) {
		...
		foreach($oeuvre_link->table as $group=>$types) {
			...
			foreach($types as $code => $libelle){
				if ((($type == 'expression') && ($oeuvre_link->attributes[$code]['EXPRESSION'] == 'yes')) || (($type == 'other_link') && ($oeuvre_link->attributes[$code]['OTHER_LINK'] == 'yes'))) {
					if(!($code == $selected))
			...
			if ($options) {
				$selector .= '
				<optgroup label="'.htmlentities($group,ENT_QUOTES,$charset).'">'.$options.'</optgroup>';
			}
		}
		$selector .= '</select>';
