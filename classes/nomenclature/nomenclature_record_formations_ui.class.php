...
// +-------------------------------------------------+
// $Id: nomenclature_record_formations_ui.class.php,v 1.22 2015-12-30 16:06:42 dgoron Exp $
				...
				if($formation['id']==$record_formation['num_formation']){
					...
					foreach ($record_formation["workshops"] as $workshop){
						...
						if($workshop['defined']){
							...
							if($workshop["label"]) $workshop_tpl.= " : &asymp;";
							else $workshop_tpl.= "&asymp;";
						}
					...
					*/
					...
					if($workshops_tpl) $workshops_tpl="<br />".$msg['nomenclature_js_workshops_label']." : ".count($record_formation["workshops"]).$workshops_tpl;
